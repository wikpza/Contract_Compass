...
import {PurchaserRepository} from "../repository/Purchaser.repository";
import {CreatePurchaserRequest, UpdatePurchaserRequest} from "../dto/Purchaser.dto";


const router = express.Router()
const repository = new PurchaserRepository()
...
router.post("/",
    async (req:Request, res:Response):Promise<any>=>{
        try{
            ...
            const result = await repository.createPurchaser(
               req.body
            )
...
router.patch("/:id",
    async (req:Request, res:Response):Promise<any>=>{
        try{
            ...
            const result = await repository.updatePurchaser(
                {...req.body, id}
            )
            return res.status(201).json(result)
        }catch(error){
...
router.get("/",
    async (req: Request, res: Response): Promise<any> => {
        try {
            ...
            const response = await repository.getPurchaser({
                page: parsedPage,
                limit: parsedLimit,
...
router.delete("/:id",
    async (req:Request, res:Response, next:NextFunction):Promise<any>=>{
        try{
            ...
            const response = await repository.deletePurchaser({id:id})
