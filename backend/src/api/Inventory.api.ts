...
import {
    AddInventorySessionRequest,
    AddProductContractRequest,
...
router.post("/",
    async (req:Request, res:Response):Promise<any>=>{
        try{
            ...
            const result = await repository.addProductContract(
               req.body
            )
...
router.get("/:id",
    async (req: Request, res: Response): Promise<any> => {
        try {
            ...
            const response = await repository.getInventory({
                page: parsedPage,
                limit: parsedLimit,
...
router.get("/history/:id",
    async (req: Request, res: Response): Promise<any> => {
        try {
            ...
            const response = await repository.getInventoryHistory({
                page: parsedPage,
                limit: parsedLimit,
                sortType: parsedSortType,
                sortBy: parsedSortBy,
...
router.delete("/:id",
    async (req:Request, res:Response, next:NextFunction):Promise<any>=>{
        try{
            ...
            const response = await repository.deleteInventory({id:id})
...
router.post("/:id",
    async (req:Request, res:Response):Promise<any>=>{
        try{
            ...
            if(errors) return res.status(400).json({message:"bad request",details:errors || {}})
            const result = await repository.addInventoryProduct(
                {...req.body, id}
            )
            return res.status(201).json(result)
        }catch(error){
