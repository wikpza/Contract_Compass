...
import {GetFileLinkType} from "@/types/FileLink.ts";
import {useDeleteFileLink, useUpdateFileLink} from "@/api/FileLink.api.ts";
...
type Props = {
    ...
    data:{count:number, rows:GetFileLinkType[]} | undefined,
    contractId:number,
    refetch: ()=>void,
}

export function FileLinkTable({data, refetch, searchParams, setSearchParams}:Props) {

    const totalPages = Math.ceil(data?.count / searchParams.limit);
    const columns = [
        {value:"name", name:"Название"},
        {value:"url", name:"Ссылка"},
        {value:"createdAt", name:"Дата создания"}
    ]
    const {update, isSuccess:isUpdateSuccess, response:updateResponse} = useUpdateFileLink()
    const {deleteObj, data:deleteResponse, isSuccess:isDeleteSuccess} = useDeleteFileLink()
                                                    ...
                                                    >
                                                        ...
                                                        <DialogContent className="bg-white text-black">
                                                            ...
                                                            <DialogHeader>
                                                                <DialogTitle>Заявители</DialogTitle>
                                                                <DialogDescription>
                                                                    {`Вы уверены, что хотите удалить Ссылку ${row.name}?`}
                                                                </DialogDescription>
                                                            </DialogHeader>
