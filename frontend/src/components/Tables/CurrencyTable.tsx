...
import {GetCurrencyType} from "@/types/Currency.ts";
import {useDeleteCurrency, useUpdateCurrency} from "@/api/Currency.api.ts";
import CurrencyForm from "@/components/Forms/CurrencyForm.tsx";

type Props = {
    searchParams:SearchParams,
    setSearchParams: (input:SearchParams)=>void
    data:{count:number, rows:GetCurrencyType[]} | undefined,
    refetch: ()=>void,
}

export function CurrencyTable({data, refetch, searchParams, setSearchParams}:Props) {

    const totalPages = Math.ceil(data?.count / searchParams.limit);
    const columns = [
        {value:"name", name:"Название"},
        {value:"code", name:"Код"},
        {value:"symbol", name:"Символ"},
        {value:"createdAt", name:"Дата создания"}
    ]
    const {update, isSuccess:isUpdateSuccess, response:updateResponse} = useUpdateCurrency()
    const {deleteObj, data:deleteResponse, isSuccess:isDeleteSuccess} = useDeleteCurrency()
                                                ...
                                                <div className="flex justify-end space-x-2">
                                                    ...
                                                    >
                                                        ...
                                                        <DialogHeader>
                                                            ...
                                                            <div>
                                                                <CurrencyForm response={updateResponse?.response} onCancel={() => setOpenDialogId(null)}  status={updateResponse?.status} onUpdate={update} data={row}/>
                                                            </div>
                                                        </DialogContent>
                                                    </Dialog>
                                                    ...
                                                    >
                                                        ...
                                                        <DialogContent className="bg-white text-black">
                                                            ...
                                                            <DialogHeader>
                                                                <DialogTitle>Валюты</DialogTitle>
                                                                <DialogDescription>
                                                                    {`Вы уверены, что хотите удалить Валюту ${row.name}?`}
                                                                </DialogDescription>
                                                            </DialogHeader>
