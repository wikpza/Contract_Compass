...
import {Edit, Trash, ArrowUp, ArrowDown, BoxSelect, Box} from 'lucide-react';
...
import {GetProductInventoryType} from "@/types/Inventory.ts";
import {getDeliveryPercentage} from "@/utils/helpers.ts";
import {Progress} from "@/components/ui/progress.tsx";
import {Tabs, TabsContent, TabsList, TabsTrigger} from "@/components/ui/tabs.tsx";
import ProductInventoryForm from "@/components/Forms/ProductInventoryForm.tsx";
import {useAddInventorySessionType, useDeleteInventory, useUpdateProductInventoryType} from "@/api/Inventory.api.ts";
import ProductDeliveryForm from "@/components/Forms/ProductDeliveryForm.tsx";
import {DeliveryProductHistoryTable} from "@/components/Tables/DeliveryProductHistoryTable.tsx";
import NoteDialog from "@/components/NoteDialog.tsx";

type Props = {
    searchParams:SearchParams,
    setSearchParams: (input:SearchParams)=>void
    data:{count:number, rows:GetProductInventoryType[]} | undefined,
    refetch: ()=>void,
    contractId:number
}

export function ProductInventoryTable({data, refetch, searchParams, setSearchParams, contractId}:Props) {

    const totalPages = Math.ceil(data?.count / searchParams.limit);
    const columns = [
        {value:"name", name:"Название"},
        {value:"contractQuantity", name:"Количество по контракту"},
        {value:"takeQuantity", name:"Полученное количество"},
        {value:"note", name:"заметка"}
    ]


    const [openDialogId, setOpenDialogId] = useState<number | null>(null);
    const [openChangeDialogId, setOpenChangeDialogId] = useState<number | null>(null);
    const [openDeleteDialogId, setOpenDeleteDialogId] = useState<number | null>(null);

    const {update, isSuccess:isUpdateSuccess, response:updateResponse} = useUpdateProductInventoryType()
    const {create:addSession, isSuccess:isAddSessionSuccess ,response:addSessionResponse} = useAddInventorySessionType()
    const {deleteObj, isSuccess:isDeleteSuccess, data:deleteInventoryData} = useDeleteInventory()
    ...
    useEffect(() => {
        if (isUpdateSuccess && updateResponse && updateResponse?.status === 201) {
            refetch();
            setOpenChangeDialogId(null);
        }

        if (isAddSessionSuccess && addSessionResponse && addSessionResponse?.status === 201) {
            refetch();
            setOpenDialogId(null);
        }

        if (isDeleteSuccess && deleteInventoryData && deleteInventoryData?.status === 201) {
            refetch();
            setOpenDeleteDialogId(null);
        }
    }, [ isUpdateSuccess, isAddSessionSuccess, isDeleteSuccess]);
                                    ...
                                    data.rows.map((row, rowIndex) => (
                                        <TableRow key={row.id || rowIndex}>
                                            ...
                                            <TableCell className={'text-center'}>
                                                    ...
                                                    {`${row.contractQuantity} ${row.product.unit.symbol}`}

                                            </TableCell>

                                            <TableCell className={'text-center'}>
                                                {`${row.takeQuantity} ${row.product.unit.symbol}`}
                                            </TableCell>
                                            ...
                                            <TableCell className={'text-right'}>
                                                ...
                                                >
                                                    <DialogTrigger asChild>
                                                        ...
                                                        >
                                                                  <span className="flex items-center">
                                                                    ...
                                                                    <span className="sr-only">доставить</span>
                                                                  </span>
                                                        </Button>
                                                    ...
                                                    <DialogContent className=" max-w-[900px]">
                                                        <DialogHeader>
                                                            <DialogTitle>Обработчик товара</DialogTitle>
                                                            <DialogDescription>
                                                               Запись продукта (получение или возврат) для  {row.product.name}
                                                            </DialogDescription>
                                                        </DialogHeader>
                                                        ...
                                                        <Tabs defaultValue="add" className="mt-4">
                                                            ...
                                                            <TabsContent value="add" className="pt-4">
                                                              <ProductDeliveryForm
                                                                  response={addSessionResponse?.response}
                                                                  onCancel={()=>setOpenDialogId(null)}
                                                                  id={row.id}
                                                                  status={addSessionResponse?.status}
                                                                  onCreate={addSession}
                                                                  />
                                                            </TabsContent>

                                                            <TabsContent value="history" className="pt-4">
                                                               <DeliveryProductHistoryTable id={row.id}/>
                                                            </TabsContent>
                                                        </Tabs>
                                                    </DialogContent>
                                                </Dialog>
                                                ...
                                                <Dialog
                                                    open={openChangeDialogId === row.id}
                                                    onOpenChange={(isOpen) => setOpenChangeDialogId(isOpen ? row.id : null)}
                                                >
                                                    <DialogTrigger asChild>
                                                    ...
                                                    <DialogContent className="max-w-3xl">
                                                        <DialogHeader>
                                                            <DialogTitle>Обраточик товара</DialogTitle>
                                                            <DialogDescription>
                                                                Запись продукта (получение или возврат) для  {row.product.name}
                                                            </DialogDescription>
                                                        </DialogHeader>
                                                        ...
                                                        <ProductInventoryForm
                                                            response={updateResponse?.response}
                                                                              onCancel={()=>setOpenChangeDialogId(null)}
                                                ...
                                                >
                                                    ...
                                                    <DialogContent className="bg-white text-black">
                                                        ...
                                                        <DialogHeader>
                                                            ...
                                                            <DialogDescription>
                                                                <span className={'flex'}>Данное действие также удалит всю историю товара для данного контракта.</span>
                                                                {`Вы уверены, что хотите удалить товар ${row.product.name} из контракта?`}
                                                            </DialogDescription>
                                                        </DialogHeader>
