...
import {CreateProjectType, GetProjectDetailType, GetProjectType, UpdateProjectType} from "@/types/Project.ts";
...
export const useGetProjects = (
    ...
    const getRequest = async (): Promise<{data?: {count: number, rows: GetProjectType[]}, status: number}> => {
        ...
        const url = `${API_BASE_URL}/project?${params.toString()}`;
    ...
    const {data, isLoading, error, refetch} = useQuery(
        ['fetchProject', input], // Ключ запроса теперь включает параметры
        getRequest,
        {retry: 1}
    );
...
export const useCreateProject = ()=>{
    ...
    const createRequest = async(input:CreateProjectType):Promise<
        {
            unit?:GetProjectType,
        ...
        const inputData:CreateProjectType = {name:input.name, currencyId:input.currencyId, finishDate:input.finishDate, startDate:input.startDate}
...
export const useUpdateProject = ()=>{
    const accessToken = localStorage.getItem('token');
    const updateUnitRequest = async(input:UpdateProjectType):Promise<
        {
            unit?:GetProjectType,
            response?: FormErrors | { message: string};
            status:number
        ...
        const response = await fetch(`${API_BASE_URL}/project/${input.id}`,
            {
                method:"PATCH",
                headers:{
                    "Content-Type": "application/json",
...
export const useDeleteProject = ()=> {
    ...
    const {
        ...
        isSuccess,
        data
    } = useMutation("DeleteProject", deleteRequest, {
        retry:0,
        onSuccess: (data) => {
            if(data && data.status && data?.status >= 200 && data?.status < 300) toast.success('Проект успешно удален');
...
export const useGetProjectDetail = (
    ...
    const getRequest = async (): Promise<{data?: GetProjectDetailType, status: number}> => {
        // Создаем объект URLSearchParams и добавляем параметры
        const params = new URLSearchParams();


        const url = `${API_BASE_URL}/project/${input.id}`;
    ...
    const {data, isLoading, refetch, isError} = useQuery(
        ['fetchProjectDetail', input], // Ключ запроса теперь включает параметры
        getRequest,
        {retry: 1}
    );
...
export const useUpdateProjectStatus = ()=>{
    const accessToken = localStorage.getItem('token');
    const updateUnitRequest = async(input:{id:number}):Promise<
        {
            unit?:GetProjectType,
            response?: FormErrors | { message: string};
            status:number
        ...
        const response = await fetch(`${API_BASE_URL}/project/status/${input.id}`,
            {
                method:"PATCH",
                headers:{
                    Authorization:`Bearer ${accessToken}`,
    ...
    const {mutate:update, isLoading, isSuccess, error, data} = useMutation(updateUnitRequest, {
        ...
        onSuccess: (data) => {
            if(data?.status >= 200 && data?.status < 300)
                toast.success("Статус проекта успешно изменен");
        },
    })
