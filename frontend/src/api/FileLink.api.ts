...
export const useGetFileLinks = (
    input: SearchParams & {contractId:number}
) => {
    const accessToken = localStorage.getItem('token');

    const getRequest = async (): Promise<{data?: {count: number, rows: GetFileLinkType[]}, status: number}> => {
        ...
        const url = `${API_BASE_URL}/doc/link?${params.toString()}`;
    ...
    const {data, isLoading, error, refetch} = useQuery(
        ['fetchFileLink', input], // Ключ запроса теперь включает параметры
        getRequest,
        {retry: 1}
    );
...
export const useCreateFileLink = ()=>{
    ...
    const createRequest = async(input:CreateFileLinkType):Promise<
        {
            unit?:GetFileLinkType,
            response?: FormErrors | { message: string};
            status:number
        }> =>{

        const response = await fetch(`${API_BASE_URL}/doc/link`,
            {
                method:"POST",
                headers:{
                    "Content-Type": "application/json",
    ...
    const {mutate:create, isLoading, isSuccess, error, data} = useMutation(createRequest, {
        ...
        onSuccess: (data) => {
            if(data?.status === 201)
                toast.success("Ссылка успешна добавлена");
        },
...
export const useUpdateFileLink = ()=>{
    const accessToken = localStorage.getItem('token');
    const updateUnitRequest = async(input:UpdateFileLinkType):Promise<
        {
            unit?:GetFileLinkType,
            response?: FormErrors | { message: string};
            status:number
        ...
        const response = await fetch(`${API_BASE_URL}/doc/link/${input.id}`,
            {
                method:"PATCH",
                headers:{
                    "Content-Type": "application/json",
    ...
    const {mutate:update, isLoading, isSuccess, error, data} = useMutation(updateUnitRequest, {
        ...
        onSuccess: (data) => {
            if(data?.status >= 200 && data?.status < 300)
                toast.success("Ссылка успешна обновлена");
        },
    })
...
export const useDeleteFileLink = ()=> {
    ...
    > =>{
        ...
        const response = await fetch(`${API_BASE_URL}/doc/link/${id}`,{
            method:"DELETE",
            headers:{
                'Content-Type': "application/json",
                Authorization:`Bearer ${accessToken}`,
    ...
    const {
        ...
        isSuccess,
        data
    } = useMutation("DeleteFileLink", deleteRequest, {
        retry:0,
        onSuccess: (data) => {
            if(data && data.status && data?.status >= 200 && data?.status < 300) toast.success('Ссылка успешна удалена');
